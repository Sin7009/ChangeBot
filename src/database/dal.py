...
# OPTIMIZATION: Store as tuple to allow zero-copy returns (tuples are immutable)
...
async def get_target_currencies(session: AsyncSession, chat_id: int) -> Sequence[str]:
    ...
    if chat_id in _settings_cache:
        ...
        if now - timestamp < CACHE_TTL:
            # OPTIMIZATION: Return the cached tuple directly (zero-copy).
            # Callers must treat this as read-only.
            return data

    # Cache miss
    settings = await get_chat_settings(session, chat_id)
    # Ensure it's a list from DB, convert to tuple for cache
