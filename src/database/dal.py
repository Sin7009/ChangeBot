...
# OPTIMIZATION: Store immutable tuple to allow zero-copy return
...
async def get_target_currencies(session: AsyncSession, chat_id: int) -> Sequence[str]:
    ...
    if chat_id in _settings_cache:
        ...
        if now - timestamp < CACHE_TTL:
            # OPTIMIZATION: Return cached tuple directly (zero-copy).
            # Caller must not mutate the returned sequence.
            return data
    ...
    # Convert to tuple for immutable caching
