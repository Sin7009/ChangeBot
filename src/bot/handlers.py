...
from src.bot.keyboards import settings_keyboard, CURRENCY_FLAGS, chart_quick_pick_keyboard
...
async def _send_chart(message: Message, currency: str):
    """
    Helper to generate and send a chart for the given currency (vs RUB).
    """
...
async def cmd_chart(message: Message, command: CommandObject):
    ...
    args = command.args
    if not args:
        # UX Improvement: Quick-Pick keyboard instead of error
        keyboard = chart_quick_pick_keyboard()
        await message.answer("📉 <b>Графики валют</b>\n\nВыберите валюту или введите код вручную (например, <code>/chart USD</code>):", reply_markup=keyboard, parse_mode="HTML")
        return
...
async def on_chart_pick(callback: CallbackQuery):
    ...
    # Remove buttons to prevent double-clicking, but keep message context
